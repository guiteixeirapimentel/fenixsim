...
FinModelDynamic::FinModelDynamic(float chordRoot, float chordTrailling, float m, float s, float thickness, ID3D11Device* pDevice)
	:
...
FinModelDynamic::~FinModelDynamic()
{
	if (cPIndexBuffer)
	{
...
ID3D11Buffer* FinModelDynamic::GetVertexBuffer() const
{
	return cPVertexBuffer;
}

ID3D11Buffer* FinModelDynamic::GetIndexBuffer() const
{
	return cPIndexBuffer;
}

unsigned int FinModelDynamic::GetIndexCount() const
{
	return cIndexCount;
}

void FinModelDynamic::ChangeFin(float chordRoot, float chordTrailling, float m, float s,
	float thickness, GraphicsDxD* pGfx)
{
	constexpr float one = 1.0f;
	constexpr float zero = 0.0f;
	constexpr float half = 1.0f / 2.0f;
	...
	HRESULT rs = pGfx->GetPDeviceContext()->Map(cPVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapSub);

	if (FAILED(rs))
	{
		MessageBox(NULL, L"Erro ao alterar fin", L"ERRO", 0);
	}

	memcpy(mapSub.pData, vertices, sizeof(VertexTypeShader3D) * 24);

	pGfx->GetPDeviceContext()->Unmap(cPVertexBuffer, 0);
}