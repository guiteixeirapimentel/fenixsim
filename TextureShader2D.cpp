...
TextureShader2D::TextureShader2D(HWND hWnd, GraphicsDxD& gfx)
	:
	Shader(hWnd, gfx.GetPDevice())
{
	...
	result = D3DCompileFromFile(L"Texture2D.vs", NULL, NULL, "TextureVertexShader2D", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, NULL, L"Texture2D.vs");
		}
	...
	result = D3DCompileFromFile(L"Texture2D.ps", NULL, NULL, "TexturePixelShader2D", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, NULL, L"Texture2D.ps");
		}
		// If there was nothing in the error message then it simply could not find the file itself.
		else
		{
			MessageBox(NULL, L"Texture2D.ps", L"Missing Shader File", MB_OK);
		}
	...
	polygonLayout[0].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;//DXGI_FORMAT_R32G32B32_FLOAT;
	...
	matrixBufferDesc.ByteWidth = sizeof(MatrixBufferTypeTextureShader2D);
