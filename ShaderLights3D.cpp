...
ShaderLights3D::ShaderLights3D(HWND hWnd, GraphicsDxD& gfx)
	:
	Shader(hWnd, gfx.GetPDevice()),
	cPPerFrameBuffer(nullptr)
{
	i = 7;
	HRESULT result;
	...
	result = D3DCompileFromFile(L"3DLightsTextVertexShader.vs", NULL, NULL, "TextureVertexLightsShader3D", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, NULL, L"3DLightsTextVertexShader.vs");
		}
		// If there was  nothing in the error message then it simply could not find the shader file itself.
		else
		{
			MessageBox(NULL, L"3DLightsTextVertexShader.vs", L"Missing Shader File", MB_OK);
		}
	...
	result = D3DCompileFromFile(L"3DLightsTextPixelShader.ps", NULL, NULL, "TexturePixelLightsShader3D", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, NULL, L"3DLightsTextPixelShader.ps");
		}
		// If there was nothing in the error message then it simply could not find the file itself.
		else
		{
			MessageBox(NULL, L"3DLightsTextPixelShader.ps", L"Missing Shader File", MB_OK);
		}
	...
	matrixBufferDesc.ByteWidth = sizeof(MatrixBufferTypeLightsShader3D);
	...
	//SETUP PERFRAME BUFFER
	perFrameBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	perFrameBufferDesc.ByteWidth = sizeof(BufferPerFrame);
	...
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result = gfx.GetPDevice()->CreateBuffer(&perFrameBufferDesc, NULL, &cPPerFrameBuffer);
	if (FAILED(result))
	{
		MessageBox(NULL, L"Erro", L"Erro ao criar bofferperframe", 0);
	}
...
ShaderLights3D::~ShaderLights3D()
{
	if (cPPerFrameBuffer)
	{
		cPPerFrameBuffer->Release();
		cPPerFrameBuffer = nullptr;
	}
}